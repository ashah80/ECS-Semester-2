from PrattParser import PrattParserMixin
from StringPool import StringPoolMixin
...
class Parser(PrattParserMixin, StringPoolMixin):
    """
    ...
    def __init__(self, tokens, vm_file):
        ...
        # String pool related attributes
        self.string_pool = {}
        self.unique_string_count = 0
        self.has_compiled_string_pool = False
    ...
    def compile_class(self):
        ...
        # Find string literals and assign them static indices for the string pool
        self.find_string_literals()
        self.compile_string_pool()
    ...
    def compile_subroutine_body(self, subroutine_name, subroutine_type):
        ...
        # Call string pool initialization function at the beginning of each subroutine
        if self.unique_string_count > 0 and not self.has_compiled_string_pool:
            self.has_compiled_string_pool = True
            write_vm_call(self.vm_file, f"{self.class_name}.stringInitialization", 0)
            write_vm_pop(self.vm_file, 'temp', 0) # discard void return value
    ...
    def compile_let_statement(self):
        ...
        if self.get_current_token()[1] == '[':
            ...
            self.compile_and_emit_expression() # pushes index
            write_vm_push(self.vm_file, var_info[1], var_info[2])
            write_vm_arithmetic(self.vm_file, 'add')
            
            self.consume_token(expected_type='symbol', expected_value=']')
            self.consume_token(expected_type='symbol', expected_value='=')
            self.compile_and_emit_expression() # pushes value to assign
        ...
        # If regular, pop the value of the expression into the variable based on its kind and index
            ...
            self.consume_token(expected_type='symbol', expected_value='=')
            self.compile_and_emit_expression()
    ...
    def compile_if_statement(self):
        ...
        self.compile_and_emit_expression() # pushes condition
    ...
    # OLD FUNCTION: This is the original compile_and_emit_expression function that doesn't handle operator precedence, which is replaced by the pratt parser implementation 
    ...
    def compile_term(self):
        ...
        """ 
        Returns integers for constant folding, but still emits VM code for all non-integerConstant terms and returns None.
        """

        if self.get_current_token()[0] == 'integerConstant': # integerConstant
            integer_value = int(self.consume_token(expected_type='integerConstant'))
            ## write_vm_push(self.vm_file, 'constant', integer_value)
            # don't emit yet, instead return the value so compile_and_emit_expression can fold it
            ...
            static_index = self.string_pool[string_value] # look up the static index for this string literal in the string pool
            write_vm_push(self.vm_file, 'static', static_index) # just push the pre-initialized static var
            return None
            ...
            self.consume_token(expected_type='symbol', expected_value='(')
            result = self.compile_expression() # might return a constant
            self.consume_token(expected_type='symbol', expected_value=')')
            return result # return the result of the expression for potential constant folding

        elif self.get_current_token()[1] in ['-', '~']: # unaryOp
            op = self.consume_token(expected_type='symbol')
            operand = self.compile_term()
            if operand is not None: # if the operand is a constant, we can fold it at compile time and just return the result without emitting any VM code
                if op == '-':
                    return -operand
                elif op == '~':
                    return ~operand # bitwise NOT
            else: # not a constant, emit normally 
            ...
            if self.peek()[1] in ['(', '.']: # subroutineCall 
                ...
                if self.get_current_token()[1] == '[':
                    var_info = self.lookup_variable(var_name)
                    self.consume_token(expected_type='symbol', expected_value='[')
                    self.compile_and_emit_expression() # pushes the index onto the stack
